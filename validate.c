...
tracewarn(const struct auth *a)
{
	...
	for (; a != NULL; a = a->parent)
		log_warnx(" ...inheriting from: %s", a->fn);
}
    ...
    const char *ski, const char *aki)
{
	...
	if (auth_find(auths, ski) != NULL) {
		log_warnx("%s: RFC 6487: duplicate SKI", fn);
		return NULL;
	}

	a = auth_find(auths, aki);
	if (a == NULL)
		log_warnx("%s: RFC 6487: unknown AKI", fn);
...
valid_ta(const char *fn, struct auth_tree *auths, const struct cert *cert)
{
	...
	if (auth_find(auths, cert->basic.ski) != NULL) {
		log_warnx("%s: RFC 6487: duplicate SKI", fn);
		return 0;
	}
...
valid_cert(const char *fn, struct auth_tree *auths, const struct cert *cert)
{
	...
	a = valid_ski_aki(fn, auths, cert->basic.ski, cert->basic.aki);
	if (a == NULL)
		return 0;
	...
	for (i = 0; i < cert->asz; i++) {
		...
		if (valid_as(a, min, max))
			continue;
		log_warnx("%s: RFC 6487: uncovered AS: %"
	...
	for (i = 0; i < cert->ipsz; i++) {
		...
		case CERT_IP_RANGE:
			...
			log_warnx("%s: RFC 6487: uncovered IP: "
			    "%s--%s", fn, buf1, buf2);
			break;
		case CERT_IP_ADDR:
...
valid_roa(const char *fn, struct auth_tree *auths, struct roa *roa)
{
	...
	a = valid_ski_aki(fn, auths, roa->eeCert.ski, roa->eeCert.aki);
	if (a == NULL)
		return 0;
	...
	for (i = 0; i < roa->ipsz; i++) {
		...
		log_warnx("%s: RFC 6482: uncovered IP: "
		    "%s", fn, buf);
		tracewarn(a);
		return 0;
	}
